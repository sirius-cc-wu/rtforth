...
use exception::{
    ...
    INVALID_NUMERIC_ARGUMENT, RESULT_OUT_OF_RANGE, RETURN_STACK_OVERFLOW, RETURN_STACK_UNDERFLOW,
    SEARCH_ORDER_OVERFLOW, STACK_OVERFLOW, STACK_UNDERFLOW, UNDEFINED_WORD, UNEXPECTED_END_OF_FILE,
};
...
pub trait Core: Sized {
    ...
    fn add_core(&mut self) {
        ...
        self.add_primitive("definitions", Core::definitions);
        self.add_primitive("get-order", Core::get_order);
    ...
    primitive! {fn patch_compilation_semanticses(&mut self) {
        // Only FORTH wordlist
        self.wordlist_mut().search_order[0] = 0;
        self.wordlist_mut().search_order_len = 1;
...
#[cfg(test)]
mod tests {
    use super::{Core, Memory, WORDLISTS};
    use exception::{
