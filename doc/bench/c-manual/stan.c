/* I changed some constants (to get loanger run times and commented
   out the main() function - Anton Ertl */
...
#define d 		 8
    ...
    /* Bubble, Quick */
#define sortelements 	 6000
#define srtelements 	 6000
...
double Getclock ()
    ...
    struct tms rusage;
    times (&rusage);
    return (double)rusage.tms_utime/(double)HZ;
    }
#endif
...
{ \
  ...
  for (i = 1; i <= 10; i += 1) xtimes[i] /= 10.0;
  ...
  fixed = fixed + puzzlebase*xtimes[5];
  floated = floated + puzzlebase*xtimes[5];
  fixed = fixed + quickbase*xtimes[6];
  floated = floated + quickbase*xtimes[6];
  fixed = fixed + bubblebase*xtimes[7];
  floated = floated + bubblebase*xtimes[7];
  fixed = fixed + treebase*xtimes[8];
  floated = floated + treebase*xtimes[8];
  fixed = fixed + mmbase*xtimes[9];
  floated = floated + fpmmbase*xtimes[9];
  ...
  printf("     FFT  %7.3f\n", xtimes[10]);
  printf("\n");
  printf("Nonfloating point composite is %7.3f\n", fixed);
  printf("Floating point composite is    %7.3f\n", floated);

  gm = 1.0;
  for ( i = 1; i <= 8; i++ ) gm *= xtimes[i];
  printf("Nonfloating geometric mean is %7.3f\n", pow(gm, 1.0/8.0));
  gm = gm * xtimes[9] * xtimes[10];
  printf("Geometric mean is             %7.3f\n", pow(gm, 1.0/10.0));
}
#endif
