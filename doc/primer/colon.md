...
之前我們看到了 `+` 、 `-` 、 `f+` 、 `f-` 等指令，這些是系統內建的指令。現在讓我們定義自己的指令。這個指令在安裝的章節已經定義過。輸入時請注意 Forth 使用空格來解析使用者的輸入，因此指令 `."` 之後要有空格，指令 `;` 之前也要有空格。因為它們都是指令。：
...
* 檢視堆疊上資料的指令：指令 `.s` 顯示資料堆疊和浮點堆疊上的數值，但不會改變堆疊的內容。
* 拋棄堆疊上資料的指令。指令 `drop` 拋棄資料堆疊最上方的整數。指令 `nip` 則拋棄最上方之下的那個整數。
* 複製堆疊上資料的指令。之前已說過 `dup` 複製了資料堆疊最上方的整數。指令 `over` 則複製最上方之下的那個整數。
...
練習：請做以下練習，並以 `.s` 檢查結果。若有必要可以輸入任何 Forth 不認得的指令，比如 `x` ，以清除堆疊。因為 Forth 在發生錯誤時會清除所有堆疊。
