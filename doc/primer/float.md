...
例十：(29/4.05) 和 7 哪一個比較大？
```
rf> 29e 4.05e f/  7e fmax  f.
7.1604938  ok
```

例十一：求中位法 -ａbs(3.2-1.7) 和 -1.4 的最小值。 
```
...
例十三：驗證 sin(1)<sup>2</sup> + cos(1)<sup>2</sup> = 1。
```
...
例十五：求 sin<sup>-1</sup>(sin(&pi;)) 和 sin<sup>-1</sup>(sin(-&pi;/2)) 。
```
...
例十六：求 cos<sup>-1</sup>(cos(&pi;)) 和 cos<sup>-1</sup>(cos(-&pi;)) 。
```
...
例十八：求向量 (1,0) 、 (0,-1) 的角度。注意 fatan2 的堆疊效應是 ( y x -- &theta; ) 。
```
rf> 0e 1e fatan2  f.
0.0000000  ok
rf> -1e 0e fatan2  f.
...
例十九：求對應弳度 0 和 -1.5707963 的二維單位向量。注意 fsincos 的堆疊效應是 ( &theta; -- sin&theta; cos&theta; ) 。
```
rf> 0e fsincos f. f.
...
| `fatan2` | ( F:&nbsp; r1 r2 -- r3 ) | 從向量 (r1, r2) 得其方向角的弳度 r3 | f-a-tan-two |
...
例二十：將 2.9 及 -2.9 轉成整數。
```
rf> 2.9e f>s .  -2.9e f>s .
2 -2  ok
```

例二十一：將整數 2 和 3 轉成浮點數後相除。
```
rf> 2 s>f  3 s>f  f/  f.
0.6666667  ok
```
