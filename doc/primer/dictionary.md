...
在上面的例子中，程式碼和資料空間容量各為 4Mb。程式空間目前沒被使用，資料碼空間使用了 4936 個位元組。
...
| Forth | 作業系統 | 資料堆疊上整數所佔位元組數 | 資料空間中整數所佔位元組數 | 浮點數所佔位元組數 | cell 的位元組數 |
|---------|---------|-----------------------|------------------------|-----------------|---------------|
...
```
指令 `dump` 顯示浮點數 1.0 並未被放在 $13222E9 的位置，而是放在之後符合浮點數對齊要求的 $13222F0 開始的 8 個位元組。
