...
0  ok
```
所以 0 不只是十進制的 0<sub>10</sub>，也用來表示假 (false)。而二進制 64 個 1，或說十六進制的 16 個 F，不只是十進制的 -1<sub>10</sub>，也表示真 (true) 。
...
例子：Forth 2012 的標準只提供了 `<` 、 `>` 、 `<>` 及 `=` 四個整數比大小的指令，並未提供 `<=` 和 `>=`。請以 `invert` 計算「 3 大於或等於 2 」的真假
```
rf> 3 2 < invert  .
-1  ok
```
...
例子：蘋果買超過 5 顆，就每顆便宜 3 元。現在買了 10 顆，請問便宜多少錢？
```
rf> 10 5 >  3 and  10 *  .
30  ok
```
...
例子：溫度大於 25 度，要打開冷氣機。以整數最右邊的位元代表冷氣機。請以 Forth 的邏輯運算計算 26 度時是否要開冷氣。
```
rf> 25e 26e f<  1 and . 
1  ok
```
...
例子：有某個調水溫的設備，當溫度比目標溫度低 3 度時要開熱水，關冷水。比目標溫度高 3 度時要開冷水，關熱水，否則熱水冷水都要開。
...
在之前冷水熱水的例子中，如果我們想知道冷水熱水的開關是否有變化，可以使用 `xor` 提令。
...
`xor` 只在位元不同時為 1 或不同時為 0 時得到 1。

例子：承之前冷水和熱水的例子，原本開關的狀態是二進制的 11<sub>2</sub>，也就是十進制的 3<sub>10</sub>，冷水和熱水都開著 。後來變成 10<sub>2</sub>，也就是十進制的 2<sub>10</sub>，請以 Forth 求出有變化的是冷水還是熱水。
```
rf> 3 2 xor  .
1  ok
```

XOR 運算也被廣泛用於錯誤偵測、類神經網路及密碼學中。在本書進階課題 [Modbus RTU](modbus.md) 中計算循環冗餘碼 (CRC) 時也使用了 XOR 運算。有興趣可以參考。

### 以 LSHIFT 和 OR 編碼

例子：色碼，會使用到 LSHIFT 和 OR

### 以 RSHIFT 和 AND 解碼

例子：色碼，會使用到 RSHIFT 和 AND
...
| `invert` | ( -- ) &emsp;              | invert |
| `<=` | ( -- ) &emsp;              | less-or-equals |
| `>=` | ( -- ) &emsp;              | greater-or-equals |
| `f>` | ( -- ) &emsp;              | f-greater-then |
| `and`    | ( -- ) &emsp;              | and |
| `or`     | ( -- ) &emsp;              | or |
| `xor`    | ( -- ) &emsp;              | xor |
| `lshift` | ( -- ) &emsp;              | l-shift |
