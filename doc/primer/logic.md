...
賦與每個位元意義後，可以對這些位元進行運算。Forth 提供了 `invert` 、 `and` 、 `or` 、 `xor` 、 `lshift` 、 `rshift` 這幾個位元運算指令。
...
Forth 程式常用計算來代替邏輯判斷，像是「如果 ... 就 n，否則 0」這樣的句子，可以使用位元運算指令 `and` 計算出來。
...
30  ok
```
例子中 `10 5 >` 判斷是否多於 5 顆，如果是就會回傳全真。全真和 3 進行 AND 運算，會得到 3。之後的 `10 *` 將每顆便宜 3 元乘於 10 顆，得到總供便宜 30 元。
...
之所以能用 and 來計算「如果 ... 就 n，否則 0」這樣的問題，就是因為 Forth 的 `true` 是每個位元都是 1 的全真。全真和 n 進行 AND 運算會得到 n。全假和 n 進行 AND 運算會得到 0。
