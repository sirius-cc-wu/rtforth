...
例十四：Forth 2012 的標準只提供了 `<` 、 `>` 、 `<>` 及 `=` 四個整數比大小的指令，並未提供 `<=` 和 `>=`。請以 `invert` 計算「 3 大於或等於 2 」的真假
```
rf> 3 2 < invert  .
-1  ok
```
...
例十五：蘋果買超過 5 顆，就每顆便宜 3 元。現在買了 10 顆，請問便宜多少錢？
```
rf> 10 5 >  3 and  10 *  .
30  ok
```
...
例十七：溫度大於 25 度，要打開冷氣機。以整數最右邊的位元代表冷氣機。請以 Forth 的邏輯運算計算 26 度時是否要開冷氣。
```
rf> 25e 26e f<  1 and . 
1  ok
```
...
例十八：以 `and` 驗證 6 大於 5 且 6 小於 7 。
```
rf> 6 5 >  6 7 <  and .
-1  ok
```

例十九：以 `and` 驗證 6 大於 5 且 6 小於 3 中至少有一項不成立。
```
rf> 6 5 >  6 3 <  and .
0  ok
```
...
例二十：有某個調水溫的設備，當溫度比目標溫度低 3 度時要開熱水，關冷水。比目標溫度高 3 度時要開冷水，關熱水，否則熱水冷水都要開。
...
例二十一：判斷 6 大於 5 和 6 小於 3 中至少有一項成立。
```
rf> 6 5 >  6 3 <  or .
-1  ok
```

例二十二：判斷 6 大於 7 和 6 小於 3 中都不成立。
```
rf> 6 7 >  6 3 <  or .
0  ok
```
...
例二十三：承之前冷水和熱水的例子，原本開關的狀態是二進制的 11<sub>2</sub>，冷水和熱水都開著 。後來變成 10<sub>2</sub>，請見下圖，
```
      11   舊的狀態
xor   10   新的狀態
-------------------
...
例二十四：已知顏色為純紅，也就是 255<sub>10</sub>，請計算其編碼。
```
rf> 255 16 lshift  h.
FF0000  ok
```
...
例二十五：已知顏色的綠色成份為十六進制的 78<sub>16</sub>，藍色成份為十六進制的 F0<sub>16</sub>，求顏色編碼。
```
...
例二十六：請問顏色 5601520<sub>10</sub> 的紅色、綠色、藍色成份各多少？
```
