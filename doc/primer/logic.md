...
例子：以 `and` 驗證 6 大於 5 且 6 小於 7 。
```
rf> 6 5 >  6 7 <  and .
-1  ok
```

例子：以 `and` 驗證 6 大於 5 且 6 小於 3 中至少有一項不成立。
```
rf> 6 5 >  6 3 <  and .
0  ok
```
...
例子：判斷 6 大於 5 和 6 小於 3 中至少有一項成立。
```
rf> 6 5 >  6 3 <  or .
-1  ok
```

例子：判斷 6 大於 7 和 6 小於 3 中都不成立。
```
rf> 6 7 >  6 3 <  or .
0  ok
```
...
例子：承之前冷水和熱水的例子，原本開關的狀態是二進制的 11<sub>2</sub>，冷水和熱水都開著 。後來變成 10<sub>2</sub>，請見下圖，
```
      11   舊的狀態
xor   10   新的狀態
-------------------
...
XOR 運算被廣泛用於錯誤偵測、類神經網路及密碼學中。在本書進階課題 [Modbus RTU](modbus.md) 中計算循環冗餘碼 (CRC) 時也使用了 XOR 運算。有興趣可以參考。
