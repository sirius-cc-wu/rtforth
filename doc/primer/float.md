...
例八：計算 10<sup>10</sup>、 10<sup>100</sup> 及 10<sup>1000</sup>
```
rf> 1e10 f.
...
例九：計算 10<sup>100</sup> + 1。
```
rf> 1e100 1e f+ f.
10000000000000002101697803323328251387822715387464188032188166609887360023982790799717755191065313280.0000000  ok
```

從例子中可以看出 10<sup>100</sup> + 1 會得到 10<sup>100</sup>。也就是說，當我們將一個很大的數字加上相對來說很小的數字時，很小的數字常就被忽略不計了。如果我們將 10<sup>100</sup> 加 1，連加 10<sup>100</sup> 次，照理來說應得到 2 &times; 10<sup>100</sup>。結果還是會得到 10<sup>100</sup>。也就是說，原本還有 16 位數的有效位數，結果只剩 0 位數的有效位數。

因此，為了保持計算的精確度，儘量維持有足夠的有效位數，當要將一群大小懸殊的數字相加時，應先對數字小的數進行加總，才加數字大的數。因此 1 + 10 + 100 + 1000 + ... + 10<sup>100</sup> 會比 10<sup>100</sup> + 10<sup>99</sup> + ... + 10 + 1 精確得多。

例五：計算 -2.0 的平方根、 0.0/0.0 以及 1.0/0.0 。
```
rf> -2e fsqrt f.
NaN  ok
rf> 0e 0e f/ f.
...
1.0/0.0 得到無限大。但是 0.0/0.0 或是 1.0/0.0 會得到 NaN (Not a Number)。NaN 有個很重要的特性，就是對它進行的幾乎所有的算術運算都會得到 NaN。這簡化了浮點運算錯誤的處理，程式不必對每個有可能出錯的運算進行檢查，只需要檢查最後的結果，就知道是否有某浮點運算出錯。
...
| `fsqrt` | ( F:&nbsp; r1 -- r2 ) | 計算 r1 的平方根 | f-sqrt |
...
例一：驗證 sin(1)<sup>2</sup> + cos(1)<sup>2</sup> = 1。
```
...
例一：求 sin<sup>-1</sup>(sin(&pi;)) 和 sin<sup>-1</sup>(sin(-&pi;/2)) 。
```
...
例一：求 cos<sup>-1</sup>(cos(&pi;)) 和 cos<sup>-1</sup>(cos(-&pi;)) 。
```
...
`fasin` 、 `facos` 、 `fatan` 的值域都無法涵蓋 360&deg; 的範圍。當有必要求得這範圍的角度時，唯一的選擇是 `fatan2`。
`fatan2` 可以從一個二維向量得到角度。而和它對應的 `fsincos` 則可以從一個角度得到一個二維單位向量。

例一：求向量 (1,0) 、 (0,-1) 的角度。注意 fatan2 的堆疊效應是 ( y x -- &theta; ) 。
```
rf> 0e 1e fatan2  f.
0.0000000  ok
rf> -1e 0e fatan2  f.
...
例一：求對應徑度 0 和 -1.5707963 的二維單位向量。注意 fsincos 的堆疊效應是 ( &theta; -- sin&theta; cos&theta; ) 。
```
rf> 0e fsincos f. f.
